...
defineProps({
    //Make sure categories are being required
    categories: {
        type: Object,
        required: true
    }
...
<!-- We will be using Vue slots to inherit AppLayout and display our custom content.-->
<template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between" v-if="$page.props.user.permissions.includes('create categories')">
                        <Link :href="route('categories.create')" class="text-white bg-indigo-500 hover:bg-indigo-700 py-2 px-4 rounded" > <!--From Jetstream and Laravel routing-->
                        CREATE CATEGORY
                        </Link>
                    </div>
                    <div class="mt-4">
                    ...
                    <div class="flex justify-between mt-2" >
                        <Link v-if="categories.current_page > 1" :href="categories.prev_page_url" class=" py-2 px-4 rounded" > <!--From Jetstream and Laravel routing-->
                            PREVIOUS
                        </Link>
                        <div v-else></div>
                        <Link v-if="categories.current_page < categories.last_page" :href="categories.next_page_url" class=" py-2 px-4 rounded" > <!--From Jetstream and Laravel routing-->
                            NEXT
                        </Link>
                        <div v-else></div>
                    </div>
